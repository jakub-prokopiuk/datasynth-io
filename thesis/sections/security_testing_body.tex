...
\begin{itemize}
    \item Frontend: Analiza początkowo wykazała obecność podatności w bibliotece \texttt{axios} (wersja $\le$ 1.3.4). W ramach działań naprawczych zaktualizowano bibliotekę do najnowszej stabilnej wersji, eliminując zidentyfikowane zagrożenia.
    \item Backend: Audyt zależności Pythonowych nie wykazał krytycznych podatności w głównych bibliotekach produkcyjnych (FastAPI, Uvicorn, SQLAlchemy).
\end{itemize}

Kod źródłowy części serwerowej został poddany analizie przy użyciu narzędzia Semgrep \cite{semgrep}.
