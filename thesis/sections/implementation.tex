...
\begin{itemize}
    ...
    \item \textbf{Ollama (\gls{LLM})}: Lokalny serwer modeli językowych, wykorzystywany do generowania danych wymagających kreatywności i kontekstu \cite{ollama}.
\end{itemize}
...
while attempts < 10:
    ...
    attempts += 1
\end{verbatim}
Temperatura (\textit{temperature}) jest hiperparametrem kontrolującym losowość generowanych odpowiedzi \gls{LLM}. Wzrost temperatury w kolejnych próbach spłaszcza rozkład prawdopodobieństwa kolejnych słów, co zwiększa ,,kreatywność'' modelu i znacząco zmniejsza szansę na wygenerowanie duplikatu.

\subsubsection*{Przetwarzanie wsadowe (Batch Processing)}
